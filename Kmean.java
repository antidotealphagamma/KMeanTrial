...
public class KMean {
	...
	public KMean(int k, int numOfItems) {
		this.k = k;
		...
		int iter = 1;
		do {
			for (int anItem : dataItems) {
				for (int c : cz) {
					row.add(abs(c- anItem));
				}
				groups.get(row.indexOf(Collections.min(row))).add(anItem);
				row.removeAll(row);
			}
